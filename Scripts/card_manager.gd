...
func highlight_card(card, hovered):
	if hovered:
		...
		#card.z_index = PLAYER_HAND_Z + 1
	else:
		card.scale = Vector2(1, 1)
		#card.z_index = PLAYER_HAND_Z

func raycast_check_for_card_slot():
	var space_state = get_viewport().world_2d.direct_space_state
	var parameters = PhysicsPointQueryParameters2D.new()
	parameters.position = get_viewport().get_camera_2d().get_global_mouse_position()
...
func get_card_with_highest_z_index(cards):
	# Assume the first card in cards array has highest z index
	var highest_z_card = cards[0].collider.get_parent()
	var highest_z_index = highest_z_card.z_index
	
	# Loop through the rest fo the cards checking for a higher z index
